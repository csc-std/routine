...
public:
	...
	TEST_METHOD (TEST_CSC_STREAM_BYTEREADER) {
		DEF<CSC::BYTE[12]> rax ;
		auto ris = ByteReader (PhanBuffer<CSC::BYTE>::make (rax)) ;
	...
	TEST_METHOD (TEST_CSC_STREAM_BYTEWRITER) {
		...
		auto wos = ByteWriter (SharedRef<FixedBuffer<CSC::BYTE>>::make (HUGESTRING_SIZE::value)) ;
		wos << rax ;
	}

	TEST_METHOD (TEST_CSC_STREAM_TEXTREADER) {
		struct wrappered_int :private Wrapped<int> {
			inline void friend_read (TextReader<STRU8> &reader) {
				reader >> wrappered_int::mData >> _GAP_ ;
			}
		} ;
		auto rax = Buffer<int ,ARGC<4>> () ;
		auto rbx = Buffer<STRU8 ,ARGC<10>> () ;
		auto ris = TextReader<STRU8> (PhanBuffer<STRU8>::make (rbx)) ;
		...
		ris >> _CAST_<Buffer<wrappered_int ,ARGC<4>>> (rax) ;
	}

	TEST_METHOD (TEST_CSC_STREAM_TEXTWRITER) {
		auto rax = Buffer<int ,ARGC<4>> () ;
		auto wos = TextWriter<STRU8> (SharedRef<FixedBuffer<STRU8>>::make (HUGESTRING_SIZE::value)) ;
		wos << rax ;
	}
} ;
} ;
