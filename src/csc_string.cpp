...
public:
	TEST_METHOD (TEST_CSC_STRING) {
		...
		_UNITTEST_ASSERT_ (_ABS_ (r11x) < VAL64 (1E-13)) ;
		const auto r12x = _PARSEVAL64S_ (String<STR> (_PCSTR_ ("1.0"))) ;
		_UNITTEST_ASSERT_ (_ABS_ (r12x - VAL64 (1.0)) < VAL64 (1E-13)) ;
		...
		const auto r5x = _PARSEVAL64S_ (String<STR> (_PCSTR_ ("-0.0122"))) ;
		_UNITTEST_ASSERT_ (_ABS_ (r5x - VAL64 (-0.0122)) < VAL64 (1E-4)) ;
		const auto r6x = _PARSEVAL64S_ (String<STR> (_PCSTR_ ("1.42E38"))) ;
		_UNITTEST_ASSERT_ (_ABS_ (r6x - VAL64 (1.42E38)) < VAL64 (1E34)) ;
		const auto r7x = _PARSEVAL64S_ (String<STR> (_PCSTR_ ("0.99287"))) ;
		_UNITTEST_ASSERT_ (_ABS_ (r7x - VAL64 (0.99287)) < VAL64 (1E-4)) ;
