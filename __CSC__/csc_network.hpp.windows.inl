...
inline exports String<STRU8> SocketStaticProc::static_make_ipv4s (const SOCKADDR &val) {
	...
	const auto r1x = _BITWISE_CAST_ (_NULL_<ARGV<SOCKADDR_IN>> () ,val) ;
	const auto r2x = _CALL_ ([&] () {
		PACK<WORD ,CHAR> ret ;
		auto &r3x = _CAST_ (_NULL_<ARGV<CSC::BYTE[_SIZEOF_ (WORD)]>> () ,r1x.sin_port) ;
		auto &r4x = _CAST_ (_NULL_<ARGV<CSC::BYTE[_SIZEOF_ (CHAR)]>> () ,r1x.sin_addr.S_un.S_addr) ;
		ByteReader<BYTE> (PhanBuffer<const CSC::BYTE>::make (r3x)) >> ret.mP1 ;
		ByteReader<BYTE> (PhanBuffer<const CSC::BYTE>::make (r4x)) >> ret.mP2 ;
		return _MOVE_ (ret) ;
	}) ;
	return StringProc::build_ipv4s (_NULL_<ARGV<STRU8>> () ,r2x) ;
}
...
inline exports SOCKADDR SocketStaticProc::static_make_socket_addr (const String<STRU8> &val) {
	...
	const auto r1x = _CALL_ ([&] () {
		...
		auto &r3x = _CAST_ (_NULL_<ARGV<CSC::BYTE[_SIZEOF_ (WORD)]>> () ,ret.sin_port) ;
		auto &r4x = _CAST_ (_NULL_<ARGV<CSC::BYTE[_SIZEOF_ (CHAR)]>> () ,ret.sin_addr.S_un.S_addr) ;
		ByteWriter<BYTE> (PhanBuffer<CSC::BYTE>::make (r3x)) << r2x.mP1 ;
		ByteWriter<BYTE> (PhanBuffer<CSC::BYTE>::make (r4x)) << r2x.mP2 ;
		return _MOVE_ (ret) ;
	}) ;
	return _BITWISE_CAST_ (_NULL_<ARGV<SOCKADDR>> () ,r1x) ;
}
...
public:
	...
	void modify_buffer (const LENGTH &rcv_len ,const LENGTH &snd_len) {
		...
		api::setsockopt (mThis->mSocket ,SOL_SOCKET ,SO_RCVBUF ,_CAST_ (_NULL_<ARGV<STRA[_SIZEOF_ (VAR32)]>> () ,r1x) ,VAR32 (_SIZEOF_ (VAR32))) ;
		const auto r2x = VAR32 (snd_len) ;
		api::setsockopt (mThis->mSocket ,SOL_SOCKET ,SO_SNDBUF ,_CAST_ (_NULL_<ARGV<STRA[_SIZEOF_ (VAR32)]>> () ,r2x) ,VAR32 (_SIZEOF_ (VAR32))) ;
	}
	...
	void read (const PhanBuffer<BYTE> &data) {
		...
		api::setsockopt (mThis->mSocket ,SOL_SOCKET ,SO_RCVTIMEO ,_CAST_ (_NULL_<ARGV<STRA[_SIZEOF_ (TIMEVAL)]>> () ,r1x) ,VAR32 (_SIZEOF_ (TIMEVAL))) ;
		const auto r2x = api::recv (mThis->mSocket ,_LOAD_ (_NULL_<ARGV<ARR<STRA>>> () ,DEPTR[data.self]) ,VAR32 (data.size ()) ,0) ;
		const auto r3x = SocketStaticProc::static_make_timeval (0) ;
		api::setsockopt (mThis->mSocket ,SOL_SOCKET ,SO_RCVTIMEO ,_CAST_ (_NULL_<ARGV<STRA[_SIZEOF_ (TIMEVAL)]>> () ,r3x) ,VAR32 (_SIZEOF_ (TIMEVAL))) ;
		//@info: state of 'this' has been changed
		if switch_once (TRUE) {
			if (r2x >= 0)
				discard ;
	...
	void write (const PhanBuffer<const BYTE> &data) {
		_DEBUG_ASSERT_ (data.size () < VAR32_MAX) ;
		const auto r1x = SocketStaticProc::static_make_timeval (mThis->mTimeout) ;
		api::setsockopt (mThis->mSocket ,SOL_SOCKET ,SO_SNDTIMEO ,_CAST_ (_NULL_<ARGV<STRA[_SIZEOF_ (TIMEVAL)]>> () ,r1x) ,VAR32 (_SIZEOF_ (TIMEVAL))) ;
		const auto r2x = api::send (mThis->mSocket ,_LOAD_ (_NULL_<ARGV<ARR<STRA>>> () ,DEPTR[data.self]) ,VAR32 (data.size ()) ,0) ;
		const auto r3x = SocketStaticProc::static_make_timeval (0) ;
		api::setsockopt (mThis->mSocket ,SOL_SOCKET ,SO_SNDTIMEO ,_CAST_ (_NULL_<ARGV<STRA[_SIZEOF_ (TIMEVAL)]>> () ,r3x) ,VAR32 (_SIZEOF_ (TIMEVAL))) ;
		//@info: state of 'this' has been changed
		if switch_once (TRUE) {
			if (r2x >= 0)
				discard ;
...
private:
	void link_confirm () {
		...
		const auto r3x = _BITWISE_CAST_ (_NULL_<ARGV<VAR32>> () ,rax.mP1) ;
		//@info: state of 'this' has been changed
...
public:
	...
	explicit Implement (const String<STRU8> &ip_addr) {
		mThis->mSocket = UniqueRef<SOCKET> ([&] (SOCKET &me) {
			...
			api::setsockopt (me ,SOL_SOCKET ,SO_REUSEADDR ,_CAST_ (_NULL_<ARGV<STRA[_SIZEOF_ (VAR32)]>> () ,r1x) ,VAR32 (_SIZEOF_ (VAR32))) ;
			api::setsockopt (me ,SOL_SOCKET ,SO_BROADCAST ,_CAST_ (_NULL_<ARGV<STRA[_SIZEOF_ (VAR32)]>> () ,r1x) ,VAR32 (_SIZEOF_ (VAR32))) ;
		} ,[] (SOCKET &me) {
			api::closesocket (me) ;
		}) ;
		if (!ip_addr.empty ())
	...
	void read (const PhanBuffer<BYTE> &data) {
		...
		_ZERO_ (rax.mP1) ;
		rax.mP2 = VAR32 (_SIZEOF_ (SOCKADDR)) ;
		const auto r3x = api::recvfrom (mThis->mSocket ,_LOAD_ (_NULL_<ARGV<ARR<STRA>>> () ,DEPTR[data.self]) ,VAR32 (data.size ()) ,0 ,DEPTR[rax.mP1] ,DEPTR[rax.mP2]) ;
		//@info: state of 'this' has been changed
		_DYNAMIC_ASSERT_ (rax.mP2 == _SIZEOF_ (SOCKADDR)) ;
	...
	void write (const PhanBuffer<const BYTE> &data) {
		...
		const auto r2x = FD_ISSET (mThis->mSocket ,DEPTR[r1x[1]]) ;
		_DYNAMIC_ASSERT_ (r2x != 0) ;
		const auto r3x = api::sendto (mThis->mSocket ,_LOAD_ (_NULL_<ARGV<ARR<STRA>>> () ,DEPTR[data.self]) ,VAR32 (data.size ()) ,0 ,DEPTR[mThis->mPeer] ,VAR32 (_SIZEOF_ (SOCKADDR))) ;
		//@info: state of 'this' has been changed
		_DYNAMIC_ASSERT_ (r3x == data.size ()) ;
	}
} ;
