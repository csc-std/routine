...
private:
	...
	public:
		...
		inline explicit LocalProc (PTR<Holder> holder ,INDEX index) :mHolder (holder) ,mIndex (index) {}

		inline void operator() () {
			_CALL_TRY_ ([&] () {
				Detail::static_execute ((*mHolder) ,mIndex) ;
			} ,[&] () {
...
private:
	...
	public:
		...
		inline explicit LocalProc (PTR<Holder> holder) :mHolder (holder) {}

		inline void operator() () {
			_CALL_TRY_ ([&] () {
				Detail::static_execute ((*mHolder)) ;
			} ,[&] () {
				_STATIC_WARNING_ ("noop") ;
			}) ;
		}
