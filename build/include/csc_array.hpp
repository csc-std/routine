...
public:
	...
	inline auto operator* () const->decltype (_NULL_<BASE> ().get (_NULL_<const ITERATOR> ())) {
...
private:
	...
	const Array<LENGTH ,SIZE> mRange ;
...
private:
	...
	inline Array<LENGTH ,SIZE> first_item () const {
		_DEBUG_ASSERT_ (mRange.length () > 0) ;
		Array<LENGTH ,SIZE> ret = Array<LENGTH ,SIZE> (mRange.length ()) ;
		ret.fill (0) ;
		return std::move (ret) ;
	}
...
public:
	...
	String add (const String &right) const {
		...
		_MEMCOPY_ (PTRTOARR[&ret.mString.self[0 + r1x]] ,right.mString.self ,r2x) ;
		return std::move (ret) ;
	}
	...
	void addto (const DEF<ITEM[_VAL]> &right) {
		...
		if (r1x + r2x <= size ()) {
			_MEMCOPY_ (PTRTOARR[&mString.self[0 + r1x]] ,PTRTOARR[&right[0]] ,r2x) ;
	...
	void addto (const String &right) {
		...
		if (r1x + r2x <= size () && mString.size () > 0) {
			_MEMCOPY_ (PTRTOARR[&mString.self[0 + r1x]] ,right.mString.self ,r2x) ;
			mString[r1x + r2x] = ITEM (0) ;
		} else {
			*this = add (right) ;
		}
...
public:
	...
	LENGTH size () const {
		return mStack.size () > 0 ? mStack.size () - 1 : 0 ;
	}

	LENGTH length () const {
		return mStack.size () > 0 ? mWrite : 0 ;
	}
...
private:
	...
	private:
		friend Priority ;
		friend Priority<KEY ,ITEM ,SIZE> ;
		KEY mKey ;
		ITEM mItem ;
	} ;

	using NodePair = PACK<KEY ,ITEM> ;
...
private:
	...
	private:
		friend Priority ;
		friend Priority<KEY ,void ,SIZE> ;
		KEY mKey ;
	} ;

	using NodePair = PACK<KEY> ;
...
private:
	...
	using NodePair = typename SPECIALIZATION_BASE::NodePair ;
	template <class _ARG>
...
public:
	...
	implicit Priority (const std::initializer_list<NodePair> &right) : Priority (right.size ()) {
		for (auto &&i : right)
			add (i) ;
	}

	LENGTH size () const {
		return mPriority.size () > 0 ? mPriority.size () - 1 : 0 ;
	}

	LENGTH length () const {
		return mPriority.size () > 0 ? mWrite : 0 ;
	}
	...
	template <class _ARG ,class = ENABLE_TYPE<std::is_same<REMOVE_CONST_TYPE<_ARG> ,Priority>::value>>
	INDEX at (const Pair<_ARG> &item) const {
	...
	inline Priority &operator<< (const NodePair &item) {
		add (std::move (item)) ;
		return *this ;
	}

	inline Priority &operator<< (NodePair &&item) {
		add (std::move (item)) ;
		return *this ;
	}
	...
	void take (NodePair &item) popping {
		_DEBUG_ASSERT_ (!empty ()) ;
	...
	inline Priority &operator>> (NodePair &item) popping {
		take (item) ;
		return *this ;
	}
	...
	void update_insert (INDEX it) {
		...
		while (TRUE) {
			...
			if (rax.mKey > mPriority[ix].mKey)
				iy = ix ;
			ix = ix + 1 ;
			if (ix < mWrite && (iy != iw ? mPriority[iy].mKey : rax.mKey) > mPriority[ix].mKey)
				iy = ix ;
			if (iy == iw)
				break ;
	...
	void compute_esort (Array<INDEX> &out ,LENGTH len) const {
		...
		while (TRUE) {
			...
			if (ix >= len)
				break ;
			INDEX iy = iw ;
			if (mPriority[0 + r1x].mKey > mPriority[out[ix]].mKey)
				iy = ix ;
			ix = ix + 1 ;
			if (ix < len && (iy != iw ? mPriority[out[iy]].mKey : mPriority[0 + r1x].mKey) > mPriority[out[ix]].mKey)
				iy = ix ;
			if (iy == iw)
				break ;
			out[iw] = out[iy] ;
...
public:
	...
	INDEX insert (INDEX index) popping {
		INDEX ret = mList.alloc (VAR_NONE) ;
		update_resize (ret) ;
...
public:
	...
	void take () {
		...
		(mFirst != VAR_NONE ? mList[mFirst].mLeft : mLast) = VAR_NONE ;
		mList.free (ix) ;
	}
	...
	INDEX insert (INDEX index) popping {
		...
		mList[ret].mLeft = r1 ;
		(r1 != VAR_NONE ? mList[r1].mRight : mFirst) = ret ;
		r1 = ret ;
		return std::move (ret) ;
	}
	...
	void pop () {
		...
		(mLast != VAR_NONE ? mList[mLast].mRight : mFirst) = VAR_NONE ;
		mList.free (ix) ;
	}
	...
	void splice (INDEX index ,INDEX ib ,INDEX jb) {
		...
		auto &r1 = index != VAR_NONE ? mList[index].mLeft : mLast ;
		mList[ib].mLeft = r1 ;
		(r1 != VAR_NONE ? mList[r1].mRight : mFirst) = ib ;
		r1 = jb ;
	}
...
private:
	...
	public:
		...
		inline void operator= (const BOOL &right) && {
			...
			const auto r2x = BYTE (r1x & (BYTE (0X01) << (mIndex % 8))) ;
			mBase.mSet[mIndex / 8] = BYTE (mBase.mSet[mIndex / 8] ^ r2x) ;
		}
...
public:
	...
	implicit Set (const std::initializer_list<NodePair> &right) : Set (right.size ()) {
		for (auto &&i : right)
			add (i) ;
	}
...
#ifdef __CSC_COMPILER_MSVC__
	template <class _ARG ,class = ENABLE_TYPE<std::is_same<REMOVE_CONST_TYPE<_ARG> ,Set>::value>>
	INDEX at (const Pair<_ARG> &item) const {
	...
	inline Set &operator<< (const NodePair &item) {
		add (std::move (item)) ;
		return *this ;
	}

	inline Set &operator<< (NodePair &&item) {
		add (std::move (item)) ;
		return *this ;
	}
...
public:
	...
	implicit HashSet (const std::initializer_list<NodePair> &right) : HashSet (right.size ()) {
		for (auto &&i : right)
			add (i) ;
	}
...
#ifdef __CSC_COMPILER_MSVC__
	template <class _ARG ,class = ENABLE_TYPE<std::is_same<REMOVE_CONST_TYPE<_ARG> ,HashSet>::value>>
	INDEX at (const Pair<_ARG> &item) const {
		return mSet.at (_OFFSET_ (&Node::mKey ,item.key)) ;
	}
#else
	...
	inline HashSet &operator<< (const NodePair &item) {
		add (std::move (item)) ;
		return *this ;
	}

	inline HashSet &operator<< (NodePair &&item) {
		add (std::move (item)) ;
		return *this ;
	}
...
public:
	...
	void add (const KEY &key ,ITEM &&item) {
		...
		if (ix == VAR_NONE) {
			...
			(mHolder->mLast != VAR_NONE ? mSet.self[mHolder->mLast].mNext : mHolder->mFirst) = ix ;
			mHolder->mLast = ix ;
			mHolder->mLength++ ;
...
public:
	...
	LENGTH capacity () const {
		return mHolder.exist () ? mSet->size () : 0 ;
	}

	LENGTH size () const {
		return mHolder.exist () ? mSet->size () - (mSet->length () - mHolder->mLength) : 0 ;
	}

	LENGTH length () const {
		return mHolder.exist () ? mHolder->mLength : 0 ;
	}
...
#ifdef __CSC_COMPILER_MSVC__
	template <class _ARG ,class = ENABLE_TYPE<std::is_same<REMOVE_CONST_TYPE<_ARG> ,SoftSet>::value>>
	INDEX at (const Pair<_ARG> &item) const {
		return mSet->at (_OFFSET_ (&Node::mKey ,item.key)) ;
	}
#else
	...
	INDEX ibegin () const {
		return mHolder.exist () ? mHolder->mFirst : VAR_NONE ;
	}
	...
	BOOL equal (const SoftSet &right) const {
		...
		INDEX iy = right.mHolder.exist () ? right.mHolder->mRoot : VAR_NONE ;
		return equal_each (right ,ix ,iy) ;
	}
	...
	inline SoftSet &operator<< (const NodePair &item) {
		add (std::move (item)) ;
		return *this ;
	}

	inline SoftSet &operator<< (NodePair &&item) {
		add (std::move (item)) ;
		return *this ;
	}
	...
	INDEX insert (const KEY &key) popping {
		...
		if (ret == VAR_NONE) {
			...
			(mHolder->mLast != VAR_NONE ? mSet.self[mHolder->mLast].mNext : mHolder->mFirst) = ret ;
			mHolder->mLast = ret ;
			mHolder->mLength++ ;
	...
	INDEX find (const KEY &key) const {
		...
		while (TRUE) {
			if (ret == VAR_NONE)
				break ;
			const auto r1x = key < mSet.self[ret].mKey ;
